    ...
    <Tabs value={activeTab || ""} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
      <div className="flex justify-between items-center border-b pr-2">
        <TabsList className="bg-transparent p-0 m-0 border-0">
          {openFiles.map((f) => (
            <TabsTrigger key={f.id} value={f.id} className="relative h-10 rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-background">
              {f.name}
              <Button variant="ghost" size="icon" className="absolute top-1/2 right-0 -translate-y-1/2 h-5 w-5 ml-2" onClick={(e) => handleCloseTab(e, f.id)}>
                <X className="w-3 h-3" />
              </Button>
            </TabsTrigger>
          ))}
        ...
        <div>
          <Button variant="ghost" size="sm" onClick={() => onSave(activeTab!)}>
            <Save className="w-4 h-4 mr-2" />
            Save
          </Button>
          <Button variant="ghost" size="sm" onClick={() => onRequestAISuggest(activeTab!, "cursor context")}>
